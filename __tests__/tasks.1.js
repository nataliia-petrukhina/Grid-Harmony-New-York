...
describe('Grid and Flexbox', () => {
    ...
    it("`grid` should have total of 4 columns", async () => {
        const containerGridTemplateColumns = await page.$eval('.container', (el) => getComputedStyle(el).gridTemplateColumns.split(' ').length);
        expect(containerGridTemplateColumns).toBe(4);
    });
    ...
    it("`grid` items' text color should be `#eee`", async () => {
        const gridItemsColor = await page.$$eval('.container  *', els => els.map((el) => getComputedStyle(el).color));
        expect(gridItemsColor).toEqual(expect.arrayContaining(['rgb(238, 238, 238)']));
    });
});
describe('Hover', () => {
